    ...
    <section className="w-full py-16 bg-white dark:bg-gray-950">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative w-full flex justify-center mb-20">
          <InfiniteSlider durationOnHover={75} gap={24} reverse>
            {flags.map((flag, index) => (
              <img
                key={index}
                src={flag.src}
                alt={`${flag.country} flag`}
                className="aspect-square w-[120px] rounded-[4px] object-contain border border-zinc-200 dark:border-zinc-800"
              />
            ))}
          </InfiniteSlider>
        </div>
        ...
        <div className="mt-20">
          ...
          <div className="relative overflow-hidden py-8">
            <InfiniteSlider duration={30} gap={32}>
              {testimonials.map((testimonial, index) => (
                <div
                  key={index}
                  className="flex-shrink-0 w-[400px] p-6 bg-white dark:bg-zinc-800 rounded-lg shadow-lg"
                >
